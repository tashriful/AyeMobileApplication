...
$(function () {
    ...
    $('#inputarInvoiceTypes').on('change', function () {
...
function selectedobj(value, ind) {
    if (ind == 'custArInv') {
        ...
        SearchCusArInvObj(value);
    }
};

function SearchCusArInvObj(objparms) {
//                                            alert('x');
...
function arReceiptClassChangeEvent(p) {
    var linvType = p.find(":selected").attr('data-invoiceType');
    $('#inputarInvoiceType').val(linvType);
